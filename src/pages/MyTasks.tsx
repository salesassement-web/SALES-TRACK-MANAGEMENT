...
import { CheckSquare, Clock, AlertTriangle, Plus, Calendar, Play, CheckCircle2, Camera, X, Save, Info, Printer } from 'lucide-react';
import { PrintSettings } from '../components/PrintSettings';
import { CircularProgress } from '../components/CircularProgress';

export const MyTasks: React.FC = () => {
    const { tasks, currentUser, addTask, updateTask, appConfig } = useApp();

    // State for Filters - Default to show ALL tasks (wide date range)
    ...
    return (
        <>
            <PrintSettings isOpen={isPrintModalOpen} onClose={() => setIsPrintModalOpen(false)} />

            <div className="space-y-6 animate-fade-in w-full print-content">
                {/* Header */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
                    ...
                    <button
                        ...
                        className="w-full md:w-auto bg-slate-800 hover:bg-slate-700 text-white px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 shadow-lg transition-colors cursor-pointer font-bold"
                    >
                        <Printer size={20} /> Print / PDF Report
                    </button>
                </div>
                ...
                <div className="hidden print:block mb-6 border-b-2 border-slate-800 pb-4">
                    <div className="flex justify-between items-end">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 uppercase tracking-wider">Daily Task Report</h1>
                            <p className="text-sm text-slate-600 font-medium">{appConfig.appName}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs text-slate-500">Supervisor: {currentUser?.fullName}</p>
                            <p className="text-xs text-slate-500">Period: {startDate} - {endDate}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print-section">
                    ...
                    <div className="lg:col-span-2 grid grid-cols-2 gap-4">
                        <div className="bg-white p-4 md:p-6 rounded-xl shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-2xl transition-shadow card">
                            ...
                            <div className="bg-blue-100 p-2 md:p-3 rounded-lg no-print">
                        ...
                        <div className="bg-white p-4 md:p-6 rounded-xl shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-2xl transition-shadow card">
                            ...
                            <div className="bg-orange-100 p-2 md:p-3 rounded-lg no-print">
                        ...
                        <div className="bg-white p-4 md:p-6 rounded-xl shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-2xl transition-shadow card">
                            ...
                            <div className="bg-purple-100 p-2 md:p-3 rounded-lg no-print">
                        ...
                        <div className="bg-white p-4 md:p-6 rounded-xl shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-2xl transition-shadow card">
                            ...
                            <div className="bg-emerald-100 p-2 md:p-3 rounded-lg no-print">
                ...
                <div className="bg-white p-6 rounded-xl shadow-xl border border-slate-100 hover:shadow-2xl transition-shadow no-print">
                ...
                {/* Tasks Table */}
                <div className="bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden hover:shadow-2xl transition-shadow card print-section">
                    <h3 className="p-4 font-bold text-slate-800 border-b border-slate-200 hidden print:block">Detailed Task List</h3>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredTasks.length > 0 ? filteredTasks.map((task, idx) => (
                                    <tr key={task.id} className="hover:bg-slate-50 break-inside-avoid">
                                        ...
                                        <td className="p-4 text-slate-600 max-w-xs truncate print:whitespace-normal" title={task.description}>{task.description}</td>
                        ...
                        <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-scale-in">
                            ...
                            <form onSubmit={handleCreateTask} className="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    >
                                        {Object.values(TaskPriority).map((p) => <option key={p as string} value={p as string}>{p as string}</option>)}
                                    </select>
                                </div>
                                <div className="pt-4">
