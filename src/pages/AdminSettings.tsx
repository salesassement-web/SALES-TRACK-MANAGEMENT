...
import { Save, Layout, Sliders, RefreshCw, Info, Plus, Trash2, Database, Loader2, Link as LinkIcon, CheckCircle2, AlertTriangle } from 'lucide-react';

export const AdminSettings: React.FC = () => {
  const { appConfig, updateAppConfig, kpiConfig, updateKPIConfig, refreshData, isLoading, connectionStatus } = useApp();
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-10 w-full">
      ...
      <div className="bg-white rounded-xl shadow-xl border border-slate-100 p-6 hover:shadow-2xl transition-shadow">
        <div className="flex items-center gap-2 mb-6">
            <Database className={connectionStatus === 'CONNECTED' ? "text-emerald-500" : connectionStatus === 'ERROR' ? "text-red-500" : "text-slate-400"} size={20}/>
            <h3 className="text-lg font-bold text-slate-800">Database Integration</h3>
        </div>

        <div className="flex flex-col md:flex-row items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-200">
            <div className="mb-4 md:mb-0">
                <div className="flex items-center gap-2 font-bold text-slate-700">
                    <div className={`w-2 h-2 rounded-full ${connectionStatus === 'CONNECTED' ? 'bg-emerald-500 animate-pulse' : connectionStatus === 'ERROR' ? 'bg-red-500' : 'bg-slate-400'}`}></div>
                    {connectionStatus === 'CONNECTED' ? 'Connected to Google Sheets' : connectionStatus === 'ERROR' ? 'Connection Error' : 'Offline'}
                </div>
                <p className="text-sm text-slate-500 mt-1 max-w-lg">
                   {connectionStatus === 'ERROR' 
                    ? "Unable to fetch data. Please check if your Google Apps Script is deployed correctly and allows 'Everyone' to access." 
                    : "Data is fetched from Google Spreadsheet. If you made manual changes in the Excel file, click sync below."}
                </p>
            </div>
            <button 
                onClick={handleSyncData}
                disabled={isLoading}
                className={`w-full md:w-auto px-6 py-3 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg transition-all disabled:opacity-70 disabled:cursor-wait
                    ${connectionStatus === 'ERROR' ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}
                `}
            >
                {isLoading ? <Loader2 size={18} className="animate-spin" /> : connectionStatus === 'ERROR' ? <AlertTriangle size={18}/> : <RefreshCw size={18} />}
                {isLoading ? "Syncing..." : connectionStatus === 'ERROR' ? "Retry Connection" : "Sync Data from Sheet"}
            </button>
        </div>
      </div>
