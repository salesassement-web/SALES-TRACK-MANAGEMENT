...
export const Dashboard: React.FC = () => {
  ...
  return (
    <>
      {/* Print Settings Modal - MOVED OUTSIDE PRINT CONTENT */}
      ...
      <div className="w-full print-content">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 no-print gap-4">
          ...
          <button 
              ...
              type="button"
              className="w-full md:w-auto bg-slate-800 hover:bg-slate-700 text-white px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 shadow-lg transition-colors z-10 cursor-pointer font-bold"
          >
              <Printer size={20} /> Generate Report PDF
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="hidden print:block mb-6 border-b-2 border-slate-800 pb-4">
            <div className="flex justify-between items-end">
                <div>
                  <h1 className="text-2xl font-bold text-slate-900 uppercase tracking-wider">Management Performance Report</h1>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-slate-500">User: {currentUser?.fullName}</p>
                </div>
            </div>
          </div>
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-xl border border-slate-100 card print-no-break h-[350px] flex flex-col">
                      ...
                      <div className="flex-1 w-full min-h-0">
                          <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={trendData}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:10}} interval={0} angle={-15} textAnchor="end"/><YAxis/><Legend/><Bar dataKey="stay" fill="#10b981" stackId="a"/><Bar dataKey="leave" fill="#ef4444" stackId="a"/></BarChart>
                          </ResponsiveContainer>
                      </div>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-xl border border-slate-100 card print-no-break h-[350px] flex flex-col">
                      ...
                      <div className="flex-1 w-full min-h-0">
                          <ResponsiveContainer width="100%" height="100%">
                              <ComposedChart data={monthlyPerformanceData}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name"/><YAxis/><Legend/><Area type="monotone" dataKey="stay" fill="#818cf8" stroke="#818cf8"/><Line type="monotone" dataKey="leave" stroke="#f43f5e" strokeWidth={3}/></ComposedChart>
                          </ResponsiveContainer>
                      </div>
                  </div>
              </div>
          ...
          <div className="print-section">
              <h3 className="text-lg font-bold text-slate-800 mb-2 border-b pb-1 uppercase hidden print:block">4. Team Detail Data</h3>
